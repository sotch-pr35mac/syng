 ...
 * Legacy rows with empty `blocks` but non-empty `text` are upgraded using plain-text heuristics.
 */
export function ensureReaderDocumentForRendering(document: ReaderDocument): ReaderDocument {
	const canonicalVersion =
		document.canonical_schema_version ?? CURRENT_CANONICAL_SCHEMA_VERSION;
