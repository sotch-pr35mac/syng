...
fn identity_two_byte_mapping() -> ByteMapping {
    ByteMapping {
...
fn predefined_cmap_mapping(name: &str) -> Option<(ByteMapping, bool)> {
    match name {
        "Identity-H" | "Identity-V" => Some((identity_two_byte_mapping(), false)),
        "UniGB-UCS2-H" | "UniGB-UCS2-V" | "UniGB-UTF16-H" | "UniGB-UTF16-V" => {
            Some((identity_two_byte_mapping(), true))
        }
        _ => None,
    }
}
    ...
    fn new(doc: &'a Document, font: &'a Dictionary) -> PdfCIDFont<'a> {
        ...
        let encoding = match encoding {
            &Object::Name(ref name) => {
                let name = pdf_to_utf8(name);
                dlog!("encoding {:?}", name);
                predefined_cmap_mapping(&name)
                    .unwrap_or_else(|| panic!("unsupported encoding {}", name))
            }
            &Object::Stream(ref stream) => {
                let contents = get_contents(stream);
                dlog!("Stream: {}", String::from_utf8(contents.clone()).unwrap());
                (adobe_cmap_parser::get_byte_mapping(&contents).unwrap(), false)
            }
            _ => { panic!("unsupported encoding {:?}", encoding)}
        };
        let (encoding, char_codes_are_unicode) = encoding;
        ...
        PdfCIDFont{doc, font, widths, to_unicode: unicode_map, encoding, char_codes_are_unicode, default_width: Some(default_width as f64) }
    }
}
...
impl<'a> PdfFont for PdfCIDFont<'a> {
    ...
    fn next_char(&self, iter: &mut Iter<u8>) -> Option<(CharCode, u8)> {
        ...
        for range in &self.encoding.cid {
            if code.0 >= range.src_code_lo && code.0 <= range.src_code_hi {
                return Some((code.0 - range.src_code_lo + range.dst_CID_lo, code.1 as u8));
            }
        }
        None
    }
    fn decode_char(&self, char: CharCode) -> String {
        ...
        if let Some(s) = s {
            s.clone()
        } else if self.char_codes_are_unicode {
            std::char::from_u32(char)
